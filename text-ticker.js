class TextTickerElement extends HTMLElement {
      ...
      try {
        ...
        if (Array.isArray(parsed)) {
          this.items = parsed.filter(item => {
            ...
            if (typeof item === 'object' && item !== null && typeof item.text === 'string' && item.text.trim().length > 0) {
              return true;
            }
            return false;
          });
    ...
    const itemsHtml = this.items
      .map(item => {
        if (typeof item === 'string') {
          return '<span class="ticker-item">' + this.escapeHtml(item) + '</span>';
        }
        ...
        if (typeof item === 'object' && item !== null && item.text) {
          ...
          if (item.url) {
            ...
            return '<a href="' + url + '" class="ticker-item ticker-link" ' + linkAttributes + '>' + text + '</a>';
          } else {
      ...
      .join('<span class="separator">' + this.escapeHtml(separator) + '</span>');

    const fullContent = itemsHtml + '<span class="separator">' + this.escapeHtml(separator) + '</span>' + itemsHtml + '<span class="separator">' + this.escapeHtml(separator) + '</span>' + itemsHtml;
  ...
  startAnimation() {
    ...
    tempDiv.style.cssText = 'position: absolute; visibility: hidden; white-space: nowrap; font-family: ' + this.getComputedStyleValue('font-family') + '; font-weight: ' + this.getComputedStyleValue('font-weight') + '; font-size: ' + this.getComputedStyleValue('font-size') + '; line-height: 1.4;';
    tempDiv.innerHTML = itemsHtml + '<span class="separator">' + this.escapeHtml(separator) + '</span>';
    ...
    if (styleEl) {
      const keyframes = '@keyframes scroll-dynamic { 0% { transform: translateX(0); } 100% { transform: translateX(-' + cycleWidth + 'px); } }';
